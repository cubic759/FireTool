...
push it in the edge and pull out when hover on it
...
class Stopwatch(QWidget):  # ANCHOR --Stopwatch
    ...
    def get_snapped_position(self, pos: QPoint):
        """Snap window to the closest screen edge based on mouse position"""
        ...
        distances = {
            "left": abs(mx - screen_rect.left()),
            "right": abs(screen_rect.right() - mx),
            "top": abs(my - screen_rect.top()),
            "bottom": abs(screen_rect.bottom() - my),
        }
        nearest_edge = min(distances, key=distances.get)
    
        if nearest_edge == "left":
            # lock left edge, follow mouse vertically
            x_snap = screen_rect.left() + 3
            y_snap = my - h // 2
    
        elif nearest_edge == "right":
            # lock right edge, follow mouse vertically
            x_snap = screen_rect.right() - w - 3
            y_snap = my - h // 2
    
        elif nearest_edge == "top":
            # lock top edge, follow mouse horizontally
            x_snap = mx - w // 2
            y_snap = screen_rect.top() + 3
    
        else:  # bottom
            # lock bottom edge, follow mouse horizontally
            x_snap = mx - w // 2
            y_snap = screen_rect.bottom() - h - 3
    
        # Clamp so the opposite edge never leaves screen
        x_snap = min(max(x_snap, screen_rect.left() + 3), screen_rect.right() - w - 3)
        y_snap = min(max(y_snap, screen_rect.top() + 3), screen_rect.bottom() - h - 3)
    
        return QPoint(x_snap, y_snap)
    ...
    def mouseMoveEvent(self, event):
        ...
        if not self.isFullScreen():
            if self.resizing:
                ...
                new_pos = self.get_snapped_position(event.globalPos())
                self.move(new_pos)
                self.is_dragging = True
    ...
    def contextMenuEvent(self, event):
        ...
        if (
            widget_under_mouse is not None
            and not widget_under_mouse in self.function_buttons and not self.is_snapped
        ):
            self.menu.exec_(event.globalPos())
    ...
    def showWindow(self):
        ...
        screen = QApplication.primaryScreen()# QApplication.screenAt(win_rect.center()) or
